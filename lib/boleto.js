...
var Boleto = function (options) {
  ...
  if (!options['data_emissao']) {
    options['data_emissao'] = dayjs().utc()
  } else {
    options['data_emissao'] = dayjs(dayjs(options['data_emissao']).utc().format('YYYY-MM-DD'))
  }

  if (!options['data_vencimento']) {
    options['data_vencimento'] = dayjs().utc().add('5', 'days')
  } else {
    options['data_vencimento'] = dayjs(dayjs(options['data_vencimento']).utc().format('YYYY-MM-DD'))
  }
