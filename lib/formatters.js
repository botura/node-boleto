...
exports.fatorVencimento = function (date) {
  const parsedDate = dayjs(date).utc().format('YYYY-MM-DD')
  const startDate = (dayjs(date).isBefore('2025-02-22')) ? dayjs('1997-10-07').utc().format('YYYY-MM-DD') : dayjs('2025-02-22').subtract(1000, 'day').utc().format('YYYY-MM-DD')
  return exports.addTrailingZeros(dayjs(parsedDate).diff(startDate, 'days'), 4)
}
